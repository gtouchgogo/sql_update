...
if [ "${db_status}" = "0" ]; then
  ...
  pgstattuple"
    ...
    # 为了select等权限不被拒
    psql  postgres://${admin_account}:${admin_password}@${default_host}:${default_port}/ejabberd -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ejabberd;" -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO startalk;"
